...
class DagmaLinear:
    ...
    def _score(self, x: np.ndarray, c: np.ndarray, W: np.ndarray, validate=False) -> typing.Tuple[float, np.ndarray]:
        r"""
    ...
    def _func(self, x:np.ndarray, c:np.ndarray, W: np.ndarray, mu: float, s: float = 1.0, validate=False) -> typing.Tuple[float, np.ndarray]:
        r"""
        ...
        """
        score, _ = self._score(x, c, W, validate)
        h, _ = self._h(W, s)
    ...
    def minimize(self, 
        ...
        for iter in range(1, max_iter+1):
            ...
            if iter % self.checkpoint == 0 or iter == max_iter:
                obj_new, score, h = self._func(self.X, self.cov, W, mu, s)
                self.vprint(f'\nInner iteration {iter}')
                self.vprint(f'\th(W_est): {h:.4e}')
                self.vprint(f'\tscore(W_est): {score:.4e}')
                self.vprint(f'\tobj(W_est): {obj_new:.4e}')
                if es_threshold!= None and np.abs((obj_prev - obj_new)/obj_prev) <= es_threshold:
                    pbar.update(max_iter-iter+1)
                    self.vprint("\tearly stop:{}".format(max_iter-iter+1))
                    break
                obj_prev = obj_new
                self.vprint(
                     "TRAINING: [Iter {}] score {:.3E}, likelihood {:.3E}, h {:.3E}".format(
                             iter, obj_new, score, h
                             )
                        )
                if self.validate is not None:
                        val_obj, val_score, val_h = self._func(self.validate, self.cov_validate, W, mu, s, True)
                        self.vprint(
                            "VALIDATION: [Iter {}] score {:.3E}, likelihood {:.3E}, h {:.3E}".format(
                                iter, val_obj, val_score, val_h
                            )
                        )
            pbar.update(1)
        return W, True
    ...
    def fit(self, 
            ...
            mu_init: float = 1.0, 
            mu_factor: float = 0.1, 
            s: typing.Union[typing.List[float], float] = [1.0, .9, .8, .7], 
            warm_iter: int = 2e4, 
            max_iter: int = 6e4, 
            lr: float = 0.0003, 
            checkpoint: int = 1000, 
        ...
        if self.loss_type == 'l2':
            diff = X.mean(axis=0, keepdims=True)
            self.X -= diff
            if self.validate is not None:
               self.validate -= diff
        ...
        if validate is not None:
           self.cov_validate = self.validate.T @ self.validate /float(self.n)  
        ...
        with tqdm(total=(T-1)*warm_iter+max_iter) as pbar:
            for i in range(int(T)):
                ...
                while success is False:
                    W_temp, success = self.minimize(self.W_est.copy(), mu, inner_iters, s[i], lr=lr_adam, beta_1=beta_1, beta_2=beta_2, pbar=pbar, es_threshold=es_threshold)
                    if success is False:
        ...
        self.score_final, _ = self._score(self.X, self.cov, self.W_est)
