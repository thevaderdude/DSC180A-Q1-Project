...
from golem_torch import golem_torch
from models.dagma.linear_dagma import DagmaLinear
from models.notears.linear import notears_linear
...
def main():
    ...
    if args.method == 'GOLEM':
       ...
       dl = DagmaLinear(args.loss)
       B_est = dl.fit(dataset.X)

    elif args.method == "NOTEARS" and args.loss is not None:
       B_est = notears_linear(dataset.X, args.lambda_1, args.loss)
