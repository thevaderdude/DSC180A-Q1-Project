...
def plot_method(method, X, B_true, B_init, B_est, B_processed, time, print_func):
    ...
    fdr, tpr, fpr, shd, pred_size = count_accuracy(B_true != 0, B_processed != 0).values()

    #aupr, curve = cdt.metrics.precision_recall(B_true, B_est)

    text = "method:{}\nfdr:{}\ntpr:{}\nfpr:{}\nshd:{}\nprediction size:{}\ntime:{}".format(method,
            fdr, tpr, fpr, shd, pred_size, time)

    plt.figtext(0, -0.5, text, ha="left", fontsize=12)
